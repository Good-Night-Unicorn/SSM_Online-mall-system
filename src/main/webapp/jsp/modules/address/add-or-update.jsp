            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑收货地址</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6 addressNameDiv">
                                        <label>收货人</label>
                                        <input style="width: 450px" id="addressName" name="addressName" class="form-control"
                                               placeholder="收货人">
                                    </div>
                                    <div class="form-group col-md-6 addressPhoneDiv">
                                        <label>电话</label>
                                        <input style="width: 450px" id="addressPhone" name="addressPhone" class="form-control"
                                               onchange="addressPhoneChickValue(this)"  placeholder="电话">
                                    </div>
                                    <div class="form-group col-md-6 addressDizhiDiv">
                                        <label>地址</label>
                                        <input style="width: 450px" id="addressDizhi" name="addressDizhi" class="form-control"
                                               placeholder="地址">
                                    </div>
                                    <div class="form-group col-md-6 isdefaultTypesDiv">
                                        <label>是否默认地址</label>
                                        <select style="width: 450px" id="isdefaultTypesSelect" name="isdefaultTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    // 查询列表
        ...
        function isdefaultTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=isdefault_types", "GET", {}, (res) => {
                if(res.code == 0){
                    isdefaultTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationIsdefaulttypesSelect(){
            var isdefaultTypesSelect = document.getElementById('isdefaultTypesSelect');
            if(isdefaultTypesSelect != null && isdefaultTypesOptions != null  && isdefaultTypesOptions.length > 0 ){
                for (var i = 0; i < isdefaultTypesOptions.length; i++) {
                    isdefaultTypesSelect.add(new Option(isdefaultTypesOptions[i].indexName,isdefaultTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(isdefaultTypesSelect != null && isdefaultTypesOptions != null  && isdefaultTypesOptions.length > 0 ) {
            for (var i = 0; i < isdefaultTypesOptions.length; i++) {
                if (isdefaultTypesOptions[i].codeIndex == ruleForm.isdefaultTypes) {//下拉框value对比,如果一致就赋值汉字
                        isdefaultTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function addressPhoneChickValue(e){
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                addressDizhi: "required",
                isdefaultTypes: "required",
            },
            messages: {
                yonghuId: "用户不能为空",
                addressName: "收货人不能为空",
    ...
    // 获取当前详情
    function getDetails() {
        var addaddress = window.sessionStorage.getItem("addaddress");
        if (addaddress != null && addaddress != "" && addaddress != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("address/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationIsdefaulttypesSelect();
            <!--  初始化级联表的下拉框  -->
