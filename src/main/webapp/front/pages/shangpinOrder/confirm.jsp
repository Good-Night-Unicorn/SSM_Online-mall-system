...
<style>
    ...
    /*.addressBox .address-item.selected .address-item-in{ border-color:var(--publicMainColor,orange);}*/
    ...
    .addressBox .address-item .selected{border-color:var(--publicMainColor,orange);}
    .style-list li.selected{border-color:var(--publicMainColor,orange);}
    ...
    .orderC-total .order-sum{color:var(--publicMainColor,orange);}
    ...
    .btn-red{display:inline-block; background:var(--publicMainColor,orange); color:#fff !important; border-radius:3px; -moz-border-radius:3px; -webkit-border-radius:3px; text-decoration:none !important;}
...
<div id="app">
		...
		<div style="height: 100px;width: 1140px;margin: 0 auto;">
			...
			<el-steps :active="2" finish-status="success" simple style="margin-top: 30px;float: right;width: 500px">
    ...
    <div class="page-order-comfirm">
        <div class="section-con">
            ...
            <div class="addressBox clearfix">
                <div class="address-item" v-for="(item,index) in addressList" :key="index" @click="addressOnChecked(index)">
                    <label>
                        <div class="address-item-in" :id="index" :class="item.isdefaultTypes == 2?'selected':''">
                            <input type="radio" name="address" :checked="item.isdefaultTypes == 2?true:false" :value="index" style="display: none">
                            <div>收件人：{{item.addressName}}</div>
                            <div>地址：{{item.addressDizhi}}</div>
                            <div>电话：{{item.addressPhone}}</div>
                        </div>
                    </label>
                </div>
            ...
            <div class="orderBox">
                <div class="orderC-table">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td class="text-left">
                                <div class="order-gw clearfix">
                                    <div class="order-img"><img :src="item.shangpinPhoto"/></div>
                                    <div class="order-sum">
                                        <h2>{{item.shangpinName}}</h2>
                                    </div>
                                </div>
                            </td>
                            <td>{{item.shangpinNewMoney}} RMB</td>
                            <td>
                                {{item.buyNumber}}
                            </td>
                            <td >
                                <span class="priceD">￥{{(item.shangpinNewMoney*item.buyNumber).toFixed(2)}}</span>
                            </td>
                        </tr>
                        </tbody>
                    </table>

                    <div class="orderC-total" v-if="this.shangpinOrderPaymentTypes == 1">
                        <div class="order-t">
                            <dl class="clearfix">
                    ...
                    <div class="orderC-total" v-if="this.shangpinOrderPaymentTypes == 2">
                        <div class="order-t">
                            <dl class="clearfix">
                                <dd class="dl_ddL">总&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;价：</dd>
                    ...
                    <div class="order-submit">
                        <a @click="payClick()" class="btn-red btn-submit">提交订单</a>
                    </div>
