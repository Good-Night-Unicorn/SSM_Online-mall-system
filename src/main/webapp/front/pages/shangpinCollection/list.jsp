            ...
            <div class="ng-inmod-ini">
                    <div class="ng-fault ng-container">
                        <div class="ng-inmod-the ng-container">
                            ...
                            <span class="ng-inmod-the-cn" aos="fade-down">商品收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="shangpinName" id="shangpinName" placeholder="商品名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                ...
                <div class="ng-inside-box ng-fault ng-container" aos="fade-up">
                    <div style="margin-top: 2em;">
                        <div v-for="(item,index) in dataList" v-bind:key="index"  class="col-md-3 list-grid">
                            <i @click="deleteShangpinCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                            <div @click="jumpCheck('../shangpin/detail.jsp?id='+item.shangpinId , item.shangpinDelete == null?'':item.shangpinDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="list-img">
                                <img :src="item.shangpinPhoto?item.shangpinPhoto.split(',')[0]:''" class="img-responsive" style="height: 350px;width:100%;" />
                                <div class="textbox"></div>
                                <h4 >名称:{{item.shangpinName}}</h4>
                                <p v-if="item.shangpinNewMoney != null" style="color: red;" >￥{{item.shangpinNewMoney}}</p>
                            </div>
                        </div>
                    </div>
                    <nav class="pages">
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            ,deleteShangpinCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('shangpinCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function shangpinCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=shangpin_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
    ...
    // 级联表的字段数据
        ...
        function pageList() {
            ...
            if (jquery('#shangpinName').val() != null && jquery('#shangpinName').val() != "" && jquery('#shangpinName').val() != "null" ) {
                param['shangpinName'] = jquery('#shangpinName').val();
            }
                                         
            var shangpinTypes = $("#shangpinTypes").find("option:selected").val();
            if (shangpinTypes != null && shangpinTypes != "" && shangpinTypes != "null") {
                            param['shangpinTypes'] = shangpinTypes;
                        }
                                                                                                                                                                                    
            // 当前表
                                                
            //收藏只能查类型为1的
            param['shangpinCollectionTypes'] = 1;

            // 获取列表数据
            http.request('shangpinCollection/page', 'get', param, function(res) {
